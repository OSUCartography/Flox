...
public final class Graph {
    ...
    public ArrayList<Flow> getSortedFlowsForDrawing(Model model, boolean increasing) {
        ...
        for (Flow flow : flows) {
            if (flow instanceof FlowPair) {
                FlowPair flowPair = (FlowPair) flow;
                sortedFlows.add(flowPair.createOffsetFlow1(model, Flow.FlowOffsetting.HIGH_QUALITY));
                sortedFlows.add(flowPair.createOffsetFlow2(model, Flow.FlowOffsetting.HIGH_QUALITY));
            } else {
